...
function processFile(input, file) {
	...
	batch_number = getBatchNumber(fft_values); //removes peak
...
function getBatchNumber(fft_values) {
/* Function takes in an array of FFT values and returns the set of 8 values preceding the spike */
	...
	max_delta_avg = 0;
	spike_index = 0;
	total_indices = fft_values.length;
	start_index = floor(total_indices*0.8);
	last_index = 0;

	partial_fft_array = Array.slice(fft_values,start_index,total_indices);
	partial_fft_array = shift(partial_fft_array);
	
	//Loop over gray-values in batches of 8 to find the one with spike (highest difference in average)
	for (i=0; i<=partial_fft_array.length-8; i=i+8) {
		cur_batch++;
		batch = Array.slice(partial_fft_array,i,i+8);
		batch_avg = avg(batch);
	...
	partial_batch_index = partial_fft_array.length-1-last_index;
	if (partial_batch_index != 0) {
		cur_batch++;
		partial_batch = Array.slice(partial_fft_array, last_index ,partial_fft_array.length);
		partial_batch_avg = avg(partial_batch);
		partial_delta_avg = abs(partial_batch_avg - prev_avg);
		if (partial_delta_avg > max_delta_avg) {
			max_delta_avg = partial_delta_avg;
			spike_index = last_index;
			spike_batch = cur_batch;
		}
	}

	true_spike_index = spike_index + start_index;
	distance = cur_batch - spike_batch; 
	print("Batch #:", spike_batch, "|", "Away from last:", distance, "|", "x-coord:", true_spike_index);
	
	new_max_batch = Array.slice(fft_values, true_spike_index-8, true_spike_index);
